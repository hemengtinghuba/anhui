...
export default (date, rule) => {
	...
	if (Object.prototype.toString.call(date) !== '[object Date]' || isNaN(date.getTime())) {//若时间戳无效则getTime返回NaN
		throw new Error("非法的时间参数");
	}
	
	
	//
	const zhWeek = ['星期天', '星期一', '星期二', '星期三','星期四', '星期五','星期六'];
	...
	const formatMedia = (time) => {
		...
		let sec = time % 60;
		let result = minus.toString().padStart(2, "0") + ":" + sec.toString().padStart(2, "0");
		return result
	}
	//匹配函数，将时间规则转换为数字
	const formatFlag = (flag) => {
		let f = "";
		switch(flag) {
			case "YY": f = year.slice(2,4);break; 
			case "YYYY": f = year;break; 
			case "M": f = month;break; 
			case "MM": f = month.padStart(2,0);break; 
			case "D": f = day;break; 
			case "DD": f = day.padStart(2,0);break; 
			case "H": f = hour;break; 
			case "HH": f = hour.padStart(2,0);break; 
			case "m": f = minu;break; 
			case "mm": f = minu.padStart(2,0);break; 
			case "s": f = sec;break; 
			case "ss": f = sec.padStart(2,0);break;
			case "week": f = zhWeek[week];break;
			case "cd": f = formatCd(date);break;
			case "media": f = formatMedia(date);break;
			default: f = "";
		}
		return f;
	}				
