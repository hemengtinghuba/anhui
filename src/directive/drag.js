...
class Drag {
    ...
    initDrag() {
        ...
        let dragList = this.el.parentNode.children;
    ...
    bindMouseDown(e) {
        e.stopPropagation();
        e.preventDefault();
        document.documentElement.addEventListener("mousemove", this.bindMouseMove);
    ...
    bindMouseMove(e) {
        ...
        this.el.style.transform = `translate3D(${this.realMoveX}px, ${this.realMoveY}px, 0)`
    }
    //绑定鼠标松开事件
    bindMouseUp(e) {
        e.stopPropagation();
        e.preventDefault();
        if (this.isDragNear) {
            let near = this.moveX - this.offsetX;
            let nearIndex = -1;
            ...
            if (nearIndex !== -1) {
                this.el.parentNode.insertBefore(this.el, this.el.parentNode.children[nearIndex]);
            }
            this.el.style.transform = `translate3D(0, 0, 0)`;
        }else {
            this._realMoveX = this.realMoveX;
            this._realMoveY = this.realMoveY;
            this.el.style.transform = `translate3D(${this._realMoveX}, ${this._realMoveY}, 0)`;
        }
        
        document.documentElement.removeEventListener("mousemove", this.bindMouseMove);
    }
}
